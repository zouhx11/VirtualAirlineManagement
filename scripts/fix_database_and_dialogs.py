    ...
    def ensure_pilot_exists(self):
        ...
        try:
            ...
            if not selected_airline or not selected_airline.strip():
                print("ℹ️ No airline selected yet, skipping pilot check")
                return
            ...
            try:
                ...
                if not pilots:
                    print("ℹ️ No pilots found, creating sample pilot")
                    try:
                        ...
                        print(f"⚠️ Could not create pilot: {e}")
                        
            except ValueError as e:
                print(f"⚠️ Airline selection issue: {e}")
    ...
    def ensure_database_structure(self):
        ...
        try:
            ...
            if not os.path.exists(user_db):
                print(f"⚠️ Database {user_db} does not exist, run initial_setup.py")
                return
            ...
            with sqlite3.connect(user_db) as conn:
                ...
                if not cursor.fetchone():
                    print("⚠️ Pilots table missing, run initial_setup.py")
                    return
                ...
                if 'airline_id' not in columns:
                    print("🔧 Adding airline_id column to pilots table...")
                    cursor.execute("ALTER TABLE pilots ADD COLUMN airline_id INTEGER DEFAULT 1")
                    conn.commit()
    ...
    def load_icons(self):
        ...
        def load_icon(file_path):
            try:
                ...
                print(f"⚠️ Error loading icon {file_path}: {e}")
                return None
